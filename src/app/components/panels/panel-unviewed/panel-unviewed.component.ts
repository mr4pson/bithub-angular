...
@Component({
  ...
  styleUrls: ['../panel.component.scss', 'panel-unviewed.component.scss'],
})
export class CPanelUnviewedComponent
  ...
  implements OnChanges, AfterViewInit
{
  ...
  get favorites(): boolean {
    return this.panelService.favorites;
  }
  set favorites(v: boolean) {
    this.panelService.setFavorites(v);
  }
  ...
  get canLoadMore(): boolean {
    return (
      this.tasks?.length /*важно,чтоб не было подгрузки до первой загрузки*/ &&
      !this.loadingMore &&
      !this.exhausted &&
  ...
  public async initTasks(): Promise<void> {
    try {
      ...
      const chunk = await this.taskRepository.loadUnviewedChunk(
        this.part,
        this.chunkLength,
        this.sortBy,
        this.sortDir,
  ...
  protected async onScroll(): Promise<void> {
    try {
      ...
      this.part++;
      const filter = {
        created_at_less: this.started_at,
  ...
  public async onViewed(): Promise<void> {
    try {
      ...
      this.viewing = true;
      await this.appService.pause(300);
      const guideIds = await this.taskRepository.viewed({
        favorites: this.favorites,
      });
      window.dispatchEvent(new Event('unviewed:reload')); // сообщение в кнопку "новое в гайдах"
      guideIds.forEach((id) =>
        window.dispatchEvent(new Event(`unviewed:unset:${id}`))
      ); // сообщение в плашки гайдов
      this.viewing = false;
      this.viewed = true;
