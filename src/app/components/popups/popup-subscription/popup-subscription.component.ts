...
export class CPopupSubscriptionComponent extends CPopupComponent implements OnInit, OnChanges {
    public tariffs: ISubscriptionTariff[] = null;
    public tariff: ISubscriptionTariff = null;
    ...
    get total(): string {return (this.tariff.price - (this.promocode ? this.tariff.price / 100 * this.promocode.discount : 0)).toFixed(2);}
    get tariffUntil(): string {
        if (!this.tariff) return "";
        const date = new Date();
        date.setDate(date.getDate() + this.tariff.period);
    ...
    private async initTariffs(): Promise<void> {
        try {
            ...
            this.tariff = this.tariffs[0] || null;
        } catch (err) {
    ...
    public async onPay(): Promise<void> {
        try {
            this.loading = true;
            this.error = null;
            const dto: IOutorderCreate = {tariff_id: this.tariff.id, code: this.promocode?.code, q: 1};
