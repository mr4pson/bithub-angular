...
export class CGuideRepository {
  ...
  public loadChunk(
    ...
    return new Promise((resolve, reject) =>
      this.dataService.guidesChunk(dto).subscribe({
        next: (res) =>
          res.statusCode === 200
            ? resolve(
                new CChunk<CGuide>(
                  res.data.map((d) => new CGuide().build(d)),
  ...
  public loadFavoritesChunk(
    ...
    return new Promise((resolve, reject) =>
      this.dataService.guidesFavoritesChunk(dto).subscribe({
        next: (res) =>
          res.statusCode === 200
            ? resolve(
                new CChunk<CGuide>(
  ...
  public loadStatChunk(
    ...
    return new Promise((resolve, reject) =>
      this.dataService.guidesStatChunk(dto).subscribe({
        next: (res) =>
          res.statusCode === 200
            ? resolve(
                new CChunk<CGuide>(
  ...
  public loadStatCompletions(guide_id: number): Promise<ICompletion[]> {
    return new Promise((resolve, reject) =>
      this.dataService.guidesStatCompletions(guide_id).subscribe({
        next: (res) =>
  ...
  public loadOne(id: number): Promise<CGuide> {
    return new Promise((resolve, reject) =>
      this.dataService.guidesOne(id).subscribe({
        next: (res) =>
          res.statusCode === 200
            ? resolve(new CGuide().build(res.data))
            : reject(res.statusCode === 404 ? res.statusCode : res.error),
        error: (err) => reject(err.message),
      })
    );
  }
  ...
  public loadProgress(id: number): Promise<number> {
    return new Promise((resolve, reject) =>
      this.dataService.guidesProgress(id).subscribe({
        next: (res) =>
          res.statusCode === 200 ? resolve(res.data) : reject(res.error),
        error: (err) => reject(err.message),
      })
  ...
  public updateFavorition(dto: IFavoritionUpdate): Promise<void> {
    return new Promise((resolve, reject) =>
      this.dataService.guidesUpdateFavorition(dto).subscribe({
