...
import { CUser, IUserAuthData, IUserEnterByToken, IUserLogin, IUserRecover, IUserRegister, IUserUpdate, IUserUpdatePassword, IUserVerify } from "src/app/model/entities/user";
...
export class CAuthService {
    ...
    private init(data: IUserAuthData): void {
        this.authData = data;
        this.load();
        this.lid = this.socketService.on(`user:reload:${this.authData.id}`, () => this.load()); // реакция на пополнение
    }
