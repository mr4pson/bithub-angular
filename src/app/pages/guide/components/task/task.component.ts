...
export class CTaskComponent implements OnInit {
    ...
    private initBehavior(): void {
        ...
        this.route.queryParams.subscribe(async params => {
            if (params['task'] === `${this.task.id}`) {
                await this.open();
                const element = await this.appService.getElementById(`task-${this.task.id}`);
                const headerHeight = window.innerWidth < 1000 ? 60 : 70;
                await this.appService.pause(400); // wait for animation (300+)
                this.appService.win.scrollTo({top: element.offsetTop - headerHeight - 15, behavior: "smooth"});
            }
        });
    }
    ...
    public async open(): Promise<void> {
        try {
            if (!this.loadedTask) {
                ...
                await this.appService.waitForImagesLoading(`#task-content-${this.task.id}`);
                this.loading = false;
            }
    ...
    private onOpenError(err: any): void {
        ...
        if (err === 410) {
            this.appService.notifyError(this.words["errors"]?.["no-parent-subscription"]?.[this.lang.slug]);
            return;
        }
    ...
    public async onToggleCompletion(event: PointerEvent): Promise<void> {
        try {
            ...
            await this.appService.pause(300);
            await this.taskRepository.updateCompletion({task_id: this.task.id, completed});
    ...
    public async onCopyLink(event: PointerEvent): Promise<void> {
        ...
        window.navigator.clipboard.writeText(`${cfg.siteUrl}/${this.lang.slug}/register/ref/${this.user.uuid}?redirect=/${this.lang.slug}/guide/${this.task.guide_id}?task=${this.task.id}`);
