...
export class CGuidePage implements OnInit {
    ...
    get title(): string {return this.guide?.name[this.lang.slug];}
    get user(): CUser {return this.authService.user;}
    get warningTxt(): string {
        return this.words["guides"]?.["ftwarning-txt"]?.[this.lang.slug]
            ?.replace(/{{site_freetasks}}/g, this.appService.settings["site-freetasks"])
    ...
    public onUpdateProgress(): void {
        this.guide.progress = parseFloat((this.guide.tasks.filter(t => t.type === "main" && t.completed).length * 100 / this.guide.tasks.filter(t => t.type === "main").length).toFixed(1));
    }
