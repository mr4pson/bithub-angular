...
export class CIndexShopPage extends CSimplePage implements OnInit {
    ...
    get shopcat_id(): number {return this.listService.shopcat_id;}
    set shopcat_id(v: number) {this.listService.shopcat_id = v;}
    ...
    get filter(): any {return {search: this.search, shopcat_id: this.shopcat_id};}
    ...
    public async initShopitems(): Promise<void> {
        try {
            ...
            const chunk = await this.shopitemRepository.loadChunk(this.part, this.chunkLength, this.sorting.sortBy, this.sorting.sortDir, this.filter);
