...
export class CIndexArticlesPage extends CSimplePage implements OnInit {
    ...
    get artcat_id(): number {return this.listService.artcat_id;}
    set artcat_id(v: number) {this.listService.artcat_id = v;}
    ...
    get filter(): any {return {search: this.search, artcat_id: this.artcat_id};}
    ...
    public async initArticles(): Promise<void> {
        try {
            ...
            const chunk = await this.articleRepository.loadChunk(this.part, this.chunkLength, this.sorting.sortBy, this.sorting.sortDir, this.filter);
