...
@Component({
    selector: "list-referees",
    templateUrl: "list-referees.component.html",
    styleUrls: ["list-referees.component.scss"],
})
export class CListRefereesComponent implements OnInit, AfterViewInit {
    ...
    private async initUsers(): Promise<void> {
        try {
            ...
            const chunk = await this.userRepository.loadRefereesChunk(this.part, this.chunkLength, this.sortBy, this.sortDir);
    ...
    protected async onScroll(): Promise<void> {
        try {     
            ...
            const chunk = await this.userRepository.loadRefereesChunk(this.part, this.chunkLength, this.sortBy, this.sortDir, filter);
