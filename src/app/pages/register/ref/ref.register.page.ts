...
export class CRefRegisterPage implements OnInit {
    ...
    get redirect(): string {return this.route.snapshot.queryParams["redirect"];}
    ...
    private async initReferer(): Promise<void> {
        try {
            ...
            const statusCode = await this.userRepository.isExists(this.route.snapshot.params["uuid"]);
    ...
    private initSEO(): void {
        this.appService.setTitle(this.words["register-ref"]?.["title"]?.[this.lang.slug]);
    }
}